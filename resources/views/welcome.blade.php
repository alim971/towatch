...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
            <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                    <div class="ml-12 dark:text-white" style="text-align: center; font-size: 2.2em">
                        Only not watched:
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        @forelse($movies as $movie)
                            <div class="p-6">
                                ...
                                <div class="ml-12 dark:text-white">
                                    <h2 style="display: inline-block" data-toggle="collapse" data-parent="#inventory-items" class="collapsed"
                                        href="#collapse{{$movie->id}}" aria-expanded="true"
                                        aria-controls="collapse{{$movie->id}}">
                                        {{$movie->name}}
                                        <i class="fa fa-chevron-circle-down cursor"></i>
                                    </h2>
                                    <form  action="{{ route('movies.destroy', $movie) }}" class="inline" style="margin-left: 15px" method="post">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <div class="collapse ml-16" id="collapse{{$movie->id}}">
                                        ...
                                        <div>
                                            Url: <a class="a" href="{{$movie->url}}" target="_blank">{{$movie->url}}</a>
                                        </div>
                                        <div>
                                            Review: <a class="a" href="{{$movie->imdb}}" target="_blank">{{$movie->imdb}}</a>
                                        </div>
                                        <div>
                                            Watched:
                                            @if($movie->watched)
                    ...
                    <div class="flex justify-center mt-4 sm:items-center">
                        ...
                        <div style="margin-left: 35px" class="ml-4 text-center text-sm text-gray-500 sm:text-right sm:ml-0">
    ...
    <script>
        ...
        $("#not_watched")[0].onchange = function() {
            $.ajax({
                url: "{{route('only')}}",
                type: 'POST',
                data: {
